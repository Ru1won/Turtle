...
catcher = c.create_arc(catcher_start_x, catcher_start_y, \
                       ...
                       start=2000, extent=140, style='arc', \
...
def check_catch():
    ...
    for egg in eggs:
        egg_x, egg_y, egg_x2, egg_y2 = c.coords(egg)
        if (catcher_x - 20) < egg_x and egg_x2 < (catcher_x2 + 20) and \
        catcher_y2 - egg_y2 < 45:
            eggs.remove(egg)
            c.delete(egg)
...
root.after(1050, create_egg)
root.after(1050, move_eggs)
root.after(1050, check_catch)
root.mainloop()
